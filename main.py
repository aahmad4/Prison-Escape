...
BG = pygame.transform.scale(pygame.image.load(os.path.join("assets", "bg_pic.png")), (WIDTH, HEIGHT))

# Defining the laser object
class Laser:
...
# Defining the general characteristics of a ship
class Ship:
    COOLDOWN = 30
...
# Defining the Player (police) ship
class Player(Ship):
    def __init__(self, x, y, health=100):
        super().__init__(x, y, health)
        self.ship_img = POLICE
...
# Defining the enemy ship
class Enemy(Ship):
    COLOR_MAP = {
                "red": (ENEMY, RED_LASER),
                "green": (ENEMY, GREEN_LASER),
                "blue": (ENEMY, BLUE_LASER)
                }
